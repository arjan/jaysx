...
%%
%% renamed from z_html_parse for jaysx project by Arjan Scherpenisse
%% <arjan@miraclethings.nl>, and added JSX curly-brace constructs to
%% parser.
...
tokenize(B, S=#decoder{offset=O}) ->
    case B of
        ...
        <<_:O/binary, "{", _/binary>> ->
            tokenize_jsx_value(B, ?INC_COL(S), [], 0);
        _ ->
...
norm({Tag, Attrs}) ->
    {norm(Tag), [{norm(K), norm_attr_value(V)} || {K, V} <- Attrs], []};
...
norm_attr_value({jsx, V}) -> {jsx, V};
norm_attr_value(V) -> iolist_to_binary(V).
...
tokenize_data(B, S=#decoder{offset=O}, Start, Whitespace) ->
    case B of
        <<_:O/binary, C, _/binary>> when (C =/= $< andalso C =/= $& andalso C =/= ${) ->
...
tokenize_jsx_value(B, S=#decoder{offset=O}, Acc, L) ->
    case B of
        <<_:O/binary>> ->
            { {jsx, iolist_to_binary(lists:reverse(Acc))}, S };
        <<_:O/binary, "}", _/binary>> when L =:= 0 ->
            { {jsx, iolist_to_binary(lists:reverse(Acc))}, ?INC_COL(S) };
        <<_:O/binary, "{", _/binary>> ->
            tokenize_jsx_value(B, ?INC_COL(S), ["{"|Acc], L+1);
        <<_:O/binary, "}", _/binary>> ->
            tokenize_jsx_value(B, ?INC_COL(S), ["}"|Acc], L-1);
        <<_:O/binary, C, _/binary>> ->
            tokenize_jsx_value(B, ?INC_COL(S), [C|Acc], L)
    end.
...
tokenize_charref(Bin, S=#decoder{offset=O}, Start) ->
    case Bin of
        ...
        <<_:O/binary, C, _/binary>> when ?IS_WHITESPACE(C)
            ...
            Len = O - Start,
            <<_:Start/binary, Raw:Len/binary, _/binary>> = Bin,
        Data = case jaysx_charref:charref(Raw) of
                       undefined ->
                           Start1 = Start - 1,
                           Len1 = Len + 1,
